...
static int get_log(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	if (!cfg.log_len) {
		...
		err = nvme_get_log(fd, cfg.log_id, cfg.namespace_id,
				   cfg.lpo, cfg.lsp, 0, cfg.rae, cfg.uuid_index,
				   NVME_CSI_NVM, cfg.log_len, log);
		if (!err) {
			if (!cfg.raw_binary) {
...
static int sanitize_log(int argc, char **argv, struct command *command, struct plugin *plugin)
{
	...
	err = nvme_get_log_sanitize(fd, true, &sanitize_log);
	if (!err)
...
static int get_feature(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_get_features(fd, cfg.feature_id, cfg.namespace_id, cfg.sel, cfg.cdw11,
			NVME_UUID_NONE, cfg.data_len, buf, &result);
	if (!err) {
...
static int copy(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	nb = argconfig_parse_comma_sep_array_short(cfg.nlbs, (short *)nlbs, ARRAY_SIZE(nlbs));
	ns = argconfig_parse_comma_sep_array_long(cfg.slbas, slbas, ARRAY_SIZE(slbas));
