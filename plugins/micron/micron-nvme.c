...
{
    struct nvme_smart_log smart_log;
    if (nvme_get_log_smart(fd, NVME_NSID_ALL, false, &smart_log) == 0) {
...
static int micron_telemetry_log(int fd, __u8 type, __u8 **data, int *logSize, int da)
{
    int err;
    ...
    if (ctrl_init)
        err = nvme_get_log_telemetry_ctrl(fd, true, 0, 512, buffer);
    else
        err = nvme_get_log_telemetry_host(fd, 0, 512, buffer);
    if (err != 0) {
    ...
    *logSize = data_area[da] * 512;
    if ((buffer = (unsigned char *)realloc(buffer, (size_t)(*logSize))) != NULL) {
        if (ctrl_init)
            err = nvme_get_log_telemetry_ctrl(fd, true, 0, *logSize, buffer);
        else
            err = nvme_get_log_telemetry_host(fd, 0, *logSize, buffer);
    }
...
static int GetFeatureSettings(int fd, const char *dir)
{
    ...
    for (i = 0; i < (int)(sizeof(fmap)/sizeof(fmap[0])); i++) {
        ...
        err = nvme_get_features(fd, fmap[i].id, 1, 0, 0, 0, len, bufp, &attrVal);
        if (err == 0) {
                                ...
                                struct plugin *plugin)
{
    ...
    err = nvme_set_features_simple(fd, fid, 1, 0, false, &result);
                                ...
                                struct plugin *plugin)
{
    ...
    if (!strcmp(opt.option, "enable")) {
	err = nvme_set_features(fd, fid, 1, 1, 0, (opt.select & 0x1), 0, 0, 0,
                                NULL, &result);
        if (err == 0) {
        ...
        err = nvme_set_features(fd, fid, 1, 0, 0, (opt.select & 0x1), 0, 0, 0,
                                NULL, &result);
        if (err == 0) {
        ...
        opt.select &= 0x3;
        err = nvme_get_features(fd, fid, 1, opt.select, 0, 0, 0, NULL, &result);
        if (err == 0) {
                                ...
                                struct plugin *plugin)
{
    ...
    for (int i = 0; i < (int)(sizeof(aVendorLogs) / sizeof(aVendorLogs[0])) && aVendorLogs[i].ucLogPage != 0; i++) {
        ...
        default:
            ...
            memset(dataBuffer, 0, bSize);
            err = nvme_get_log_simple(fd, aVendorLogs[i].ucLogPage, bSize, dataBuffer);
